...
export const Pricing: React.FC<PricingProps> = ({ onOpenAuth }) => {
  ...
  const plans = [
    ...
    },
    {
      ...
      popular: true,
      buttonStyle: 'bg-gradient-to-r from-[#0050FF] to-[#00D6FF] text-white shadow-[0_0_30px_rgba(0,214,255,0.5)] hover:scale-105',
    },
    {
      ...
      <div className="max-w-[1700px] mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, index) => (
            ...
            >
              {plan.popular && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-[#0050FF] to-[#00D6FF] text-white font-mono text-[10px] font-bold uppercase tracking-widest shadow-lg flex items-center gap-1.5">
                  <Sparkles className="w-3 h-3" /> Most Popular for Students
                </div>
              )}
